 ...
 * @returns 기초 데미지 (난수 전)
 */
export function calculateBaseDamage(level: number, power: number, attack: number, defense: number): number {
...
export function calculateCatchChance(
    ...
    const b = 65536 / (255 / X) ** 0.25;
    const catchProb = (b / 65536) ** 4 * 100;
...
export function getMoveForGen(m: any, genId: number) {
    ...
    const _futureChanges = m.changelog.filter((c: any) => c.genId > genId).sort((a: any, b: any) => b.genId - a.genId);
