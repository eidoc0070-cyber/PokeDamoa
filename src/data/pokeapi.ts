...
/** 테스트 환경에서 모듈 수준 캐시를 완전히 초기화하는 함수 */
export function resetPokeapiCache(): void {
    cachedData = null;
...
export async function preloadAllData(
    ...
    if (!tabs || tabs.length === 0) {
        // 탭 정보가 없으면 전부 다운로드
        for (const k of ["pokedex", "moves", "abilities", "items", "statuses"] as DataKey[]) neededKeys.add(k);
        ...
        for (const tabId of visibleTabIds) {
            ...
            for (const k of deps) neededKeys.add(k);
        }
    ...
    for (let i = 0; i < imgTotal; i += CHUNK_SIZE) {
        ...
        await Promise.all(chunk.map((url) => fetch(url, { mode: "no-cors", cache: "force-cache" }).catch(() => {})));
    }
