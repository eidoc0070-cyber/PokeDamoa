...
import { fetchMovesData, fetchPokedexData, fetchStatusData } from "../../data/pokeapi.js";
...
import { executeEffects, executeTurn } from "./engine.js";
import type { AILevel, BattleAction, BattlePokemon, BattleState } from "./types.js";
...
export async function renderBattleAi(container: HTMLElement): Promise<() => void> {
    ...
    try {
                ...
                if (aliveIdx !== -1) {
                    ...
                    if (nextPoke) {
                        ...
                        battleState.logs.push({
                            type: "switch",
                            message: `${opponentSide.name}은(는) ${newName}을(를) 내보냈다!`,
                        });
        ...
        const handlePlayerAction = (action: BattleAction) => {
            ...
            if (playerActive?.isFainted && action.type === "switch") {
                ...
                if (nextPoke && !nextPoke.isFainted) {
                    ...
                    battleState.logs.push({
                        type: "switch",
                        message: `${battleState.player.name}은(는) ${newName}을(를) 내보냈다!`,
                    });
                        ...
                        <div style="max-height:180px; overflow-y:auto; border:1px solid #eee; border-radius:8px; padding:4px; display:flex; flex-direction:column; gap:4px;">
                            ${
                                ...
                                `;
                                          ...
                                          .join("")
                                    : '<p style="text-align:center; color:#888; padding:20px;">저장된 파티가 없습니다.<br/>파티 빌더에서 파티를 먼저 만들어주세요.</p>'
                            }
                        </div>
        ...
        const openSwitchModal = () => {
            ...
            const pSide = battleState.player;
            ...
            modalOverlay.innerHTML = `
                <div class="card" style="width: 100%; max-width: 400px; background: #fff; border-radius: 16px; padding: 20px; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 16px;">
                        <h3 style="margin:0; font-size:1.1rem;">🔄 교체할 포켓몬 선택</h3>
                        <button id="btn-close-switch-modal" style="border:none; background:none; font-size:1.2rem; cursor:pointer; color:#888;">✕</button>
                    </div>
                    <div style="display:flex; flex-direction:column; gap:10px; max-height:60vh; overflow-y:auto;">
                        ${pSide.team
                            .map((p, idx) => {
                                const isCurrent = idx === pSide.activeIdx;
                                ...
                                const statusText = p.status
                                    ? battleState?.statusData?.[p.status]?.nameKo || p.status
                                    : "";

                                return `
                                <div class="switch-candidate" data-idx="${idx}" style="
                                    padding: 12px; border-radius: 10px; border: 2px solid ${isCurrent ? "var(--primary-color)" : "#eee"};
                                    background: ${isCurrent ? "rgba(var(--primary-rgb), 0.05)" : isFainted ? "#f5f5f5" : "#fff"};
                                ...
                                ">
                                    <div>
                                        <div style="font-weight:bold; font-size:0.95rem;">
                                            ${p.data.nameKo}
                                            ${isCurrent ? '<span style="font-size:0.75rem; background:var(--primary-color); color:#fff; padding:2px 6px; border-radius:8px; margin-left:6px;">배틀 중</span>' : ""}
                                            ${isFainted ? '<span style="font-size:0.75rem; background:#757575; color:#fff; padding:2px 6px; border-radius:8px; margin-left:6px;">기절</span>' : ""}
                                            ${statusText ? `<span style="font-size:0.75rem; background:#e91e63; color:#fff; padding:2px 6px; border-radius:8px; margin-left:4px;">${statusText}</span>` : ""}
                                        </div>
                                        <div style="font-size:0.8rem; color:#666; margin-top:4px;">
                                            Lv. ${p.level} • ${p.data.types.join("/")}
                                        </div>
                                    </div>
                                    <div style="text-align:right; min-width:80px;">
                                        <div style="font-size:0.85rem; font-weight:bold; color:${hpColor};">${p.currentHp} / ${p.maxHp}</div>
            ...
            modalOverlay.querySelector("#btn-close-switch-modal")?.addEventListener("click", () => {
                document.body.removeChild(modalOverlay);
            });

            modalOverlay.querySelectorAll(".switch-candidate").forEach((el) => {
                el.addEventListener("click", () => {
                    const idx = parseInt(el.getAttribute("data-idx")!, 10);
                    if (idx === pSide.activeIdx || pSide.team[idx]?.isFainted) return;
                    document.body.removeChild(modalOverlay);
        ...
        const renderUI = () => {
            ...
            const pHpPct = Math.max(0, Math.round((playerActive.currentHp / playerActive.maxHp) * 100));
            const oHpPct = Math.max(0, Math.round((opponentActive.currentHp / opponentActive.maxHp) * 100));
            const pHpColor = pHpPct > 50 ? "#4caf50" : pHpPct > 20 ? "#ff9800" : "#f44336";
            const oHpColor = oHpPct > 50 ? "#4caf50" : oHpPct > 20 ? "#ff9800" : "#f44336";

            const pStatusText = playerActive.status
                ? battleState.statusData?.[playerActive.status]?.nameKo || playerActive.status
                : "";
            const oStatusText = opponentActive.status
                ? battleState.statusData?.[opponentActive.status]?.nameKo || opponentActive.status
                : "";

            container.innerHTML = `
                <div style="display:flex; flex-direction:column; gap:15px; height:100%;">
                    <!-- 상대방 영역 -->
                    <div class="card" style="border-left: 5px solid #f44336; position:relative;">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <div>
                                <strong>${opponentActive.data.nameKo}</strong>
                                ${oStatusText ? `<span style="font-size:0.75rem; background:#e91e63; color:#fff; padding:2px 6px; border-radius:8px; margin-left:6px;">${oStatusText}</span>` : ""}
                            </div>
                            <span style="font-size:0.85rem; color:#666;">Lv. ${opponentActive.level}</span>
                        </div>
                        <div style="background:#eee; height:10px; border-radius:5px; margin-top:8px; overflow:hidden;">
                            <div style="background:${oHpColor}; width:${oHpPct}%; height:100%; transition: width 0.3s;"></div>
                        </div>
                        <div style="text-align:right; font-size:0.8rem; margin-top:4px;">${opponentActive.currentHp} / ${opponentActive.maxHp}</div>
                    </div>

                    <!-- 로그 영역 -->
                    <div id="battle-log" class="card" style="flex:1; overflow-y:auto; background:#f9f9f9; font-size:0.88rem; padding:12px; min-height:160px; border-radius:12px; display:flex; flex-direction:column; gap:6px;">
                        ${battleState.logs
                            .map((log) => {
                                let borderStyle = "#eee";
                                let textColor = "#333";
                                ...
                                return `<div style="padding:6px 10px; border-left:3px solid ${borderStyle}; background:${bgStyle}; color:${textColor}; border-radius:4px;">${log.message}</div>`;
                            })
                            .join("")}
                    </div>
                    ...
                    <div class="card" style="border-left: 5px solid #2196f3;">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <div>
                                ...
                                ${pStatusText ? `<span style="font-size:0.75rem; background:#e91e63; color:#fff; padding:2px 6px; border-radius:8px; margin-left:6px;">${pStatusText}</span>` : ""}
                            </div>
                            <span style="font-size:0.85rem; color:#666;">Lv. ${playerActive.level}</span>
                        </div>
                        <div style="background:#eee; height:10px; border-radius:5px; margin-top:8px; overflow:hidden;">
                            <div style="background:${pHpColor}; width:${pHpPct}%; height:100%; transition: width 0.3s;"></div>
                        </div>
                    ...
                    <div class="card" style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                        ${
                            battleState.isFinished
                                ? `
                            <button id="btn-reset-battle" class="btn btn-primary" style="grid-column: span 2; padding:14px; font-weight:bold;">다시 하기</button>
                        `
                                : `
                            ${
                            ...
                            `
                                    : playerActive.moves
                                          .map(
                                              (move, i) => `
                                <button class="btn btn-move" data-idx="${i}" style="padding:12px; font-size:0.9rem; font-weight:bold;">${move.nameKo}</button>
                            `,
                                          )
                                          .join("")
                            }
                            <button id="btn-open-switch" class="btn" style="grid-column: span 2; margin-top:6px; padding:12px; font-weight:bold; ${playerActive.isFainted ? "border:2px solid var(--primary-color); background:rgba(var(--primary-rgb),0.1);" : ""}">🔄 포켓몬 교체</button>
                        `
                        }
                    </div>
                </div>
