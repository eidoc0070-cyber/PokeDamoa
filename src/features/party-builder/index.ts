import { fetchPokedexData, fetchMovesData, fetchItemsData, fetchAbilitiesData } from '../../data/pokeapi.js';
...
export async function renderPartyBuilder(container: HTMLElement): Promise<() => void> {
    ...
    try {
        const [pokemon, moves, items, abilities] = await Promise.all([
            fetchPokedexData(), fetchMovesData(), fetchItemsData(), fetchAbilitiesData()
        ]);
        ...
        const render = () => {
            ...
            if (currentView === 'list') {
                renderPartyList(content, parties, 
                    (party) => {
                ...
                renderPartyEditor(content, editingParty, ctx,
                    (updatedParty) => {
...
function createNewParty(): Party {
    ...
    return {
        ...
        name: '새로운 파티',
        members: Array(6).fill(null).map(() => createEmptySlot()),
        createdAt: now,
        updatedAt: now
    };
}
