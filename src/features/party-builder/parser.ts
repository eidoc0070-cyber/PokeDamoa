...
const STAT_NAME_MAP: Record<string, StatKey> = {
    ...
    'Atk.': 'atk', 'Def.': 'def', 'Sp. Atk': 'spa', 'Sp. Def': 'spd',
    '체력': 'hp', '공격': 'atk', '방어': 'def', '특수공격': 'spa', '특수방어': 'spd', '스피드': 'spe'
};
    ...
    for (const set of sets) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (levelMatch && levelMatch[1]) {
                slot.level = parseInt(levelMatch[1]);
                continue;
            }
            ...
            if (natureMatch && natureMatch[1]) {
                ...
                const nature = NATURES.find(n => n.nameEn.toLowerCase() === nName.toLowerCase() || n.nameKo === nName);
    ...
    for (const part of parts) {
        ...
        if (match) {
            ...
            if (match[1] !== undefined && isNaN(parseInt(match[1]))) {
                name = match[1];
                val = parseInt(match[2] || '0');
            } else {
                val = parseInt(match[1] || '0');
                name = match[2] || '';
            }
