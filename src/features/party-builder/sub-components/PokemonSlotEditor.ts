...
import { NATURES, TYPE_NAMES_KO, TYPE_COLORS, type StatKey, type PokemonType } from '../../../data/constants.js';
import { createElement } from '../../../utils/dom.js';
import { getSortedMovesForPoke, getMoveItemStyle, renderMoveItemExtra } from '../../../utils/pokemon-math.js';
...
export function renderPokemonSlotEditor(options: SlotEditorOptions) {
        ...
        <div class="slot-editor card" style="padding: 15px; margin-bottom: 20px; border: 1px solid #ddd; border-radius: 12px; background: var(--surface-color);">
            ...
            <div class="stats-section" style="margin-top:20px;">
                ...
                <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:10px;">
                    ${(['hp', 'atk', 'def', 'spa', 'spd', 'spe'] as StatKey[]).map(s => `
    ...
    const pokeInput = createAutocomplete({
        ...
        label: '포켓몬', placeholder: '이름 검색', data: ctx.pokemon,
    ...
    const itemInput = createAutocomplete({
        container: container.querySelector('#item-search-box')!,
        label: '도구', placeholder: '도구 검색', data: ctx.items,
        initialValue: ctx.items.find(i => i.id === slot.itemId)?.nameKo,
        getSearchKey: i => i.searchKey, getDisplayName: i => i.nameKo, getDisplaySub: i => `(${i.nameEn})`,
        onSelect: i => {
            slot.itemId = i.id;
            onUpdate(slot);
        }
    ...
    const abilityInput = createAutocomplete({
        container: container.querySelector('#ability-search-box')!,
        label: '특성', placeholder: '특성 검색', data: ctx.abilities,
        initialValue: ctx.abilities.find(a => a.id === slot.abilityId)?.nameKo,
        getSearchKey: a => a.searchKey, getDisplayName: a => a.nameKo, getDisplaySub: a => `(${a.nameEn})`,
        onSelect: a => {
            slot.abilityId = a.id;
            onUpdate(slot);
        }
    ...
    const moveInputs = [0, 1, 2, 3].map(i => {
        return createAutocomplete({
            ...
            label: `기술 ${i + 1}`, placeholder: '기술 검색', data: getSortedMovesForPoke(ctx.moves, ctx.pokemon.find(p => p.id === slot.pokemonId), 9),
    ...
    container.querySelector('.nature-select')?.addEventListener('change', (e) => {
        slot.natureId = parseInt((e.target as HTMLSelectElement).value);
        onUpdate(slot);
    });

    container.querySelector('.level-input')?.addEventListener('change', (e) => {
        slot.level = parseInt((e.target as HTMLInputElement).value) || 100;
        onUpdate(slot);
    });
    ...
    container.querySelectorAll('.ev-input').forEach(el => {
        el.addEventListener('change', (e) => {
            ...
            slot.evs[stat] = parseInt((e.target as HTMLInputElement).value) || 0;
            onUpdate(slot);
        });
    });
