...
import { DEFAULT_TABS, getExternalLinks, saveSettings } from "../../state/storage.js";
...
export function renderSettings(container: HTMLElement) {
  ...
  `;
    ...
    generationSelect.addEventListener("change", (e) => {
        ...
        const parsed = Number.isNaN(Number(val)) ? val : Number(val);
    ...
    btnOfflineSync.addEventListener("click", async () => {
        ...
        if (
            !confirm(
                `활성화된 탭에 필요한 데이터를 다운로드합니다. (네트워크 환경에 따라 데이터가 소모될 수 있습니다.)${skipNote}`,
            )
        )
            return;
                ...
                item.addEventListener("drop", (e) => {
                    ...
                    if (draggedIndex !== null && draggedIndex !== targetIndex) {
                        ...
                        if (draggedTab) {
                            ...
                            // 순서 변경도 커스터마이징으로 간주
                            for (const t of currentTabs) t.isCustomized = true;
