...
import type { BattleState, BattlePokemon, BattleAction, EffectTag } from '../../src/features/battle-ai/types.js';
import type { PokemonData, MoveData } from '../../src/data/pokeapi.js';
...
describe('Advanced Battle Engine Tests', () => {
    ...
    describe('Turn Order & Priority', () => {
        it('Switch should have higher priority than moves', () => {
            const playerPoke = createTestPokemon('Player', { calculatedStats: { hp: 200, atk: 100, def: 100, spa: 100, spd: 100, spe: 10 } }); // Slow
            const opponentPoke = createTestPokemon('Opponent', { calculatedStats: { hp: 200, atk: 100, def: 100, spa: 100, spd: 100, spe: 100 } }); // Fast
    ...
    describe('Move Effects & Hooks', () => {
        it('should cancel move if onBeforeMove sets cancel flag (Flinch/Paralysis)', () => {
            ...
            expect(nextState.logs.some(l => l.message.includes('전광석화 사용') || l.message.includes('몸통박치기 사용'))).toBe(true); 
            // Wait, Opponent should move. Player should NOT have "사용" log for their move.
            const playerMoveLog = nextState.logs.some(l => l.message.includes('Player') && l.message.includes('사용'));
            expect(playerMoveLog).toBe(false);
            const opponentMoveLog = nextState.logs.some(l => l.message.includes('Opponent') && l.message.includes('사용'));
            expect(opponentMoveLog).toBe(true);
        });

        it('should handle multiple effects in order of priority', () => {
            const lowPrioTag: EffectTag = {
                id: 'low', trigger: 'onTurnEnd', action: 'damage', params: { percent: 10 }, target: 'self', priority: 1, sourceType: 'system'
            };
            const highPrioTag: EffectTag = {
                id: 'high', trigger: 'onTurnEnd', action: 'heal', params: { percent: 10 }, target: 'self', priority: 10, sourceType: 'system'
            };
        ...
        it('should execute secondary effects of attacking moves', () => {
            ...
            const burnMove: MoveData = { ...dummyMove, nameKo: '불꽃세례', power: 40, effectTags: [secondaryEffect] as any };
        ...
        it('Burn status should reduce physical attack damage by 50%', () => {
            ...
            const nextStateNormal = executeTurn(stateNormal, 
                { type: 'move', side: 'player', moveIdx: 0 },
                { type: 'move', side: 'opponent', moveIdx: 0 }
            );

            const burnDmgLog = nextState.logs.find(l => l.type === 'damage' && l.message.includes('Opponent'));
            const normalDmgLog = nextStateNormal.logs.find(l => l.type === 'damage' && l.message.includes('Opponent'));

            const burnDmg = parseInt(burnDmgLog!.message.match(/\d+/)![0]);
            const normalDmg = parseInt(normalDmgLog!.message.match(/\d+/)![0]);
        ...
        it('should execute secondary rank-up effects of attacking moves', () => {
            ...
            const attackAndBoostMove: MoveData = { ...dummyMove, nameKo: '니트로차지', power: 50, effectTags: [secondaryRankTag] as any };
    ...
    describe('Faint Handling', () => {
        it('should not allow fainted pokemon to move', () => {
            ...
            const faintedMoveLog = nextState.logs.some(l => l.message.includes('Fainted') && l.message.includes('사용'));
